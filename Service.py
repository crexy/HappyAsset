...
class service:
    ...
    # 종목정보 리스트 조회
    def searchStockInfoList(self,page, limit, stock_keyword=None, business_keyword=None, customer_keyword=None, product_keyword=None):
        # S-RIM 값 조회 쿼리
        ...
        for docStock in crsCrop:  # 종목정보
            ...
            if ("A" + stock_code) in stockDB.SP_DB.list_collection_names():
                docPrices = stockDB.SP_DB["A" + stock_code].find({}).sort("날짜", -1).limit(limit)  # 가격정보 sort('year', 1)

                dictRslt['market_cap'] = docPrices[0]["시가총액"]  # 시가총액
